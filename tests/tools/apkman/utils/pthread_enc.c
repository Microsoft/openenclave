...
void oe_enclave_thread_launch_ecall(uint64_t host_thread_id,
...
static int _pthread_join_hook(pthread_t enc_thread, void** retval)
{
    ...
    while (!thread->done)
    {
	OE_CHECK(oe_host_thread_join_ocall(&r, thread->host_thread_id) == OE_OK);
    }
